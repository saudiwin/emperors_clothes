...
In this paper, we apply a tool known as randomized response [@vandenhout2002; @blair2015], and specifically the crosswise variant [@gingerich2016], to obtain valid measures of opposition to the Tunisian dictator Kais Saied. While this survey design has existed for decades, it has not been previously applied to the estimation of public opinion in authoritarian regimes [@blair2020]. As we show, randomized response has excellent performance at overcoming sensitivity bias because its assumptions are easier to justify for the particular challenge of obtaining responses from dissidents in an authoritarian regime. This sensitive survey design effectively encrypts respondent identities in a manner that the respondent can directly observe, increasing the credibility of the survey design, while also being easy to deploy via secure online survey platforms.
...
There may of course be situations where a list experiment can perform better than a randomized response design, and other situations where only a list experiment is practical, such as with a verbal phone survey. However, we believe that expanding the toolkit available to scholars will only help to address this important issue of survey bias, and we show in this paper why a particular variant of randomized response is well-suited to the issue of political authoritarianism. Furthermore, we provide a novel Bayesian parameterization of the randomized response design that allows us to obtain interesting findings regarding the nature of sensitivity bias in Middle Eastern countries.

We next turn to a brief definition of the randomized response technique as we employ it in this paper. This variant has been amply described in the literature [@vandenhout2002; @blair2015]. Both list experiments and randomized response share a similar design goal of using naturally-occurring variation, i.e., randomness, to encrypt a response within a survey. This type of encryption is similar to how modern digital cryptography works by disguising data with random noise [@gennaro2006]. Crucially, for encryption to work, the respondent must be the only one to have the key, that is, to be able to decrypt the responses by knowing what variation is due to the non-sensitive randomness and what variation is due to the sensitive trait. For a list experiment, this randomness is due to randomly showing or not showing a sensitive item in a list, while for randomized response, the respondent is asked to answer the question truthfully conditional on a random outcome like a coin flip.

Because only the respondent can observe the outcome of the coin flip, their response is effectively encrypted. However, the difficulty in implementing this type of randomization is obvious in a survey context. For in-person surveys, dice, coins, or rotating sliders can be used because the respondent can see the random process occurring [@gingerich2009]. Interviewers may need to carry a die, coin, or other type of physical randomization device, and the respondent must be able to roll the die with plausible privacy. It is not surprising that, despite its theoretical advantages, the RRT has been applied relatively infrequently in the social sciences.

Furthermore, it is impossible to use a physical randomization device in an online survey. While it is logistically simple to draw a random number in an online survey, it is not possible for the respondent to observe that privately draw *and* know that it was truly random. These practical limitations of the conventional RRT are the most likely reason why the RRT design has rarely if ever been used in the burgeoning field of online survey methods [@neundorf2021; @rosenzweig2020].

# Our Approach

It is useful to briefly review the landscape of available techniques for measuring sensitive attitudes and behaviors in survey research. Below is a summary of the main techniques used in the literature, highlighting their respective advantages and limitations. While each approach offers certain benefits, they also involve tradeoffs between privacy protection, statistical efficiency, and implementation complexity which motivated our choice of the crosswise model design for this study.
...
sensitivity_methods <- data.frame(
  Method = c(
    "Direct Questioning",
    "List Experiment (Unmatched Count)",
  ...
  Description = c(
    "Standard survey question asking directly about sensitive behavior or attitude",
    "Respondents report total number of items from a list that apply to them; sensitive item included for treatment group only",
    "Respondents use randomizing device (e.g., coin flip, dice) to determine whether to answer truthfully or give predetermined response",
    "Respondents indicate support for policy when endorsed by different political figures or groups",
    "Respondents answer whether sum of sensitive and non-sensitive items meets condition (crosswise) or report item sum directly"
  ),
  
  Advantages = c(
    "Simple, intuitive; no additional complexity for respondents or researchers",
    "Strong privacy protection; respondents never reveal individual status; relatively straightforward analysis",
    "Individual-level privacy protection; flexible implementation; can estimate prevalence",
    "Less threatening framing; useful for policy attitudes; captures partisan cueing effects",
    "Individual-level privacy through ambiguity; simpler than RRT; no randomizing device needed"
  ),
  
  Disadvantages = c(
    "High social desirability bias; potential for strategic misreporting; baseline for comparison only, Potential for self-censorship due to fear of repression or consequences ",
    "No individual-level estimates; requires larger sample sizes; assumes no design effects or liars",
    "Requires understanding of randomization; potential for non-compliance; perceived complexity may affect responses",
    "Limited to policy/attitude questions; may not fully eliminate sensitivity; requires credible endorsers",
    "Cognitive burden and potential confusion; limited validation studies compared to list experiments"
  )
)

kable(sensitivity_methods, 
      caption = "Comparison of Survey Techniques for Sensitive Questions",
      booktabs = TRUE) %>%
...
Our aim in this paper is to show the power of the RRT design by applying it to a difficult problem, which is the estimation of popular opposition to the regime of Tunisian President Kais Saied. Tunisia, which some observers thought was on a steady path to a consolidated democracy in the 2010s, succumbed to an autogolpe (self-coup) when President Saied shut down the country's elected parliament in 2021. Since then, elections for the presidency and the parliament have become less competitive due to irregularities and restrictions on candidates who want to compete. In addition, Saied has implemented heavy-handed measures to go after dissidents and rival politicians, including prosecuting critics in military courts [@marzouki2025; @kubinec2023; @petkanas2023; @koehler2023].
...
We do not observe strong non-linear dynamics with ideological distance in either @tbl-dissent or @tbl-bias. If anything, the weak negative quadratic relationship in @tbl-bias would imply that the respondents with the most distance from Saied also have the least bias in reporting opposition. However, it is important to note that there is substantial noise in these findings with confidence intervals that cross zero, and for that reason should be interpreted with caution.
...
max_func <- function(d,c,x_hat, x_s) {
...
# make x_hat exogenous
# moderates

x_prime <- 0.5

mf1 <- max_func(2,1.1,x_prime,1)

x_dprime <- -1.5

mf2 <- max_func(2,1.1,x_dprime,1)
